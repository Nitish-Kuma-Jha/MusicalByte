...
const SongList = ({ songs, onSelecteSong, currentIndex
 }) => {
        ...
        <table className="songlist-table">
          ...
          <tbody>
            {songs.map((song, index) => (
              <tr key={song.id} onClick={() => onSelecteSong(index)} className={`songlist-row ${currentIndex === index ? "songlist-row-active" : ""}`}>
